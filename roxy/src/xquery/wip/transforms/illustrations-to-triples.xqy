...
for $concept in xdmp:directory( "http://lds.org/semantic/ootr/topic/" )//fact-set[t/p = "http://lds.org/thing#scope" and t/o = "New Testament Illustrations"]
...
let $entity-type := local:set-entity-type( $concept/t[p = "http://lds.org/thing#entity-type"]/o/text() )
let $member := local:set-member( $concept/t[p = "http://lds.org/communication#type"]/o/text() )
let $rels :=
...
let $triples :=
    <sem:triples rels="{ $rel-count }">
        ...
        <sem:triple>
            ...
            <sem:object>http://lds.org/concept-scheme/illustrations</sem:object>
        </sem:triple>
        <sem:triple>
            <sem:subject>{ $s }</sem:subject>
...
let $path := fn:concat( "http://lds.org/sem/core/illustrations/", $base-uri, ".xml" )
