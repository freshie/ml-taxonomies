...
let $name := fn:tokenize( $iri, $delimiter )[fn:last()]
...
let $triples :=
    <sem:triples xmlns:sem="http://marklogic.com/semantics">
        <sem:triple>
            ...
            <sem:object>http://www.w3.org/2002/07/owl#Class</sem:object>
        </sem:triple>
        <sem:triple>
            <sem:subject>{ $iri }</sem:subject>
...
let $err-iri-exists :=
    if ( fn:exists( xdmp:directory( $model, "infinity" )//sem:triple[
        ...
        sem:object = "http://www.w3.org/2002/07/owl#Class"] ) )
        then 1 else 0
...
return
    if ( fn:sum( $errors) > 0 )
        then xdmp:redirect-response( fn:concat( "class-add.xqy", $response ) )
        else (xdmp:document-insert( $path, $triples ),xdmp:redirect-response( "class-add.xqy" ))
